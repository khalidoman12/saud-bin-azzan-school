...
/**
 * @param {Record<string, any>} teacher
 * @param {string} query
 */
export function scoreTeacher(teacher, query) {
  return scoreStudent({ ...teacher, enrollmentNumber: null }, query);
}

/**
 * All timetable filters must match the same lesson, not different parts of a teacher's week.
 * @param {Record<string, any>} lesson
 * @param {{grade?: number|null, subject?: string|null, day?: string|null, period?: number|null}} filters
 */
export function teacherLessonMatches(lesson, filters = {}) {
  return (filters.grade == null || lesson.grade === filters.grade)
    && (filters.subject == null || lesson.subject === filters.subject)
    && (filters.day == null || lesson.day === filters.day)
    && (filters.period == null || (lesson.periodStart <= filters.period && lesson.periodEnd >= filters.period));
}

/**
 * @param {Array<Record<string, any>>} teachers
 * @param {{query?: string, grade?: number|null, subject?: string|null, day?: string|null, period?: number|null}} filters
 */
export function filterAndRankTeachers(teachers, filters = {}) {
  const query = String(filters.query ?? "").trim();
  const grade = filters.grade ?? null;
  ...
  const matches = teachers
    .filter((teacher) => {
      if (grade === null && subject === null && day === null && period === null) return true;
      return teacher.lessons.some((lesson) => teacherLessonMatches(lesson, { grade, subject, day, period }));
    })
    .map((teacher) => ({ teacher, score: scoreTeacher(teacher, query) }))
    .filter(({ score }) => score > 0);
  ...
  return matches
    ...
    .sort((a, b) => {
      if (query && b.score !== a.score) return b.score - a.score;
      return a.teacher.sourcePage - b.teacher.sourcePage;
    });
}
