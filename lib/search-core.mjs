...
export function filterAndRankTeachers(teachers, filters = {}) {
  ...
  const matches = teachers
    .filter((teacher) => grade === null || teacher.grades.includes(grade))
    .filter((teacher) => subject === null || teacher.subjects.includes(subject))
    .filter((teacher) => {
      if (day === null && period === null) return true;
      return teacher.lessons.some((lesson) =>
        (day === null || lesson.day === day)
        && (period === null || (lesson.periodStart <= period && lesson.periodEnd >= period)),
      );
    })
